...
- **의존성 상태:** 현재 build.gradle은 검증된 구현 범위 기준으로 최소 정리되어 있습니다.
...
notification 운영 정책은 현재 `AUTO_RETRY`, `MANUAL_INTERVENTION`, `IGNORE` 분기와 admin recovery 기준까지 문서화되어 있으며, 채널별 세부 운영 규칙 고도화는 후속 과제로 유지합니다.
