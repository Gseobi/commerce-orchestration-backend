...
- **의존성 메모:**  
  MVC + JPA + Kafka + Flyway + WebClient + Modulith test support가 같이 보여 폭이 넓어 보일 수 있지만, 현재 코드에 직접 사용 흔적이 없는 batch, oauth2 client/server, thymeleaf, webservices, spring-boot-admin, kafka-streams 계열은 제거했습니다.
- **현재 로컬 검증:**  
