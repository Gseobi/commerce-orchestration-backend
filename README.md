...
- **강점:**  
  explicit state transition, compensation 분리, notification retry/manual/ignore 정책, outbox retry/dead-letter, admin reprocessing, idempotency/claim 기반 reliability hardening
- **거래/연동 관점:**  
...
- `paymentRequestId` 기준 결제 멱등성으로 동일 승인 요청 replay 시 provider 중복 호출을 방지합니다.
- settlement 실패는 payment cancel compensation 흐름으로 처리합니다.
- notification 실패는 `AUTO_RETRY`, `MANUAL_INTERVENTION`, `IGNORE` 정책으로 분기합니다.
- `RETRY_SCHEDULED` notification event는 `PROCESSING` claim 이후 due processor를 통해 재처리할 수 있습니다.
- outbox를 통해 후속 이벤트 발행을 분리하고, `READY` / `RETRY_WAIT` event를 `PROCESSING`으로 선점한 뒤 publish합니다.
- Outbox 상태 관리와 Kafka 발행 구현을 `OutboxEventPublisher` adapter로 분리했습니다.
...
최근 보강에서는 주문 이후 흐름에서 발생할 수 있는 중복 요청과 재처리 동시 실행 가능성을 줄이기 위해 `paymentRequestId` 기반 결제 멱등성, notification/outbox `PROCESSING` claim, Outbox publisher adapter 분리를 추가했습니다.

- **Payment Idempotency:** 같은 `paymentRequestId`로 승인 요청이 반복되면 provider를 다시 호출하지 않고 기존 payment 기준 응답을 반환합니다.
- **Notification Retry Claim:** due `RETRY_SCHEDULED` event를 `PROCESSING`으로 선점한 실행자만 retry 처리를 수행하고, 선점 실패는 skipped로 집계합니다.
...
- outbox는 `READY / RETRY_WAIT -> PROCESSING -> PUBLISHED / RETRY_WAIT / DEAD_LETTER` 상태를 사용합니다.
...
- 발행 전 DB 조건부 update로 `PROCESSING` 상태를 선점한 이벤트만 publish합니다.
- 실제 Kafka 발행은 `OutboxEventPublisher` 구현체인 `KafkaOutboxEventPublisher`가 담당합니다.
...
`NotificationRetryProcessor`는 due retry 대상만 조회하고, 처리 전 `PROCESSING` claim을 획득한 이벤트만 처리합니다.

- 성공 시 notification event를 `SENT`로 전환하고 order를 `COMPLETED`로 복구합니다.
- future retry event는 처리하지 않습니다.
- claim에 실패한 due event는 중복 실행으로 보고 `skippedCount`로 집계합니다.
...
주문 이후에는 payment, settlement, notification, outbox publish처럼  
서로 다른 실패 가능성과 복구 기준을 가진 후속 흐름이 이어집니다.

이 흐름을 각 service에 흩뿌리면  
어디까지 성공했고, 어디서 실패했으며, 어떤 단위로 복구할 수 있는지 파악하기 어렵습니다.

그래서 `CommerceOrchestrationService`를 중심으로 전체 흐름을 읽히게 만들고,  
각 domain은 공개 API 계약을 통해 협력하도록 분리했습니다.

### 2. 왜 실패를 하나로 처리하지 않았는가

payment 실패, settlement 실패, notification 실패, outbox publish 실패는  
주문 전체에 미치는 영향이 다릅니다.

payment 실패는 주문 완료를 막아야 하지만,  
notification 실패는 주문 자체를 되돌리기보다 retry, manual intervention, ignore 정책으로 분리하는 편이 더 자연스럽습니다.

따라서 실패를 단순 exception으로만 처리하지 않고,  
상태와 재처리 기준으로 남기는 구조를 선택했습니다.

### 3. 이 프로젝트가 증명하는 Backend 역량

- 상태 전이 중심의 거래 흐름 설계
- 실패 지점별 보상 / 재시도 / 수동 개입 분리
- Outbox 기반 후속 이벤트 발행 분리
- Spring Modulith 기반 module boundary 검증
- Testcontainers 기반 PostgreSQL / Kafka integration test
- 운영자가 복구할 수 있는 admin reprocessing 흐름
...
- `AUTO_RETRY`  
  ...
  운영자는 `POST /api/admin/notification-events/retry-due`로 동일 trigger를 수동 실행할 수 있고, batch 처리 결과 요약과 처리된 event id 목록을 함께 반환합니다.
- `MANUAL_INTERVENTION`  
  운영자 확인이 필요하며 notification event를 `MANUAL_INTERVENTION_REQUIRED`로 남깁니다.
- `IGNORE`  
  현재 범위에서 주문 완료를 막지 않아도 되는 실패로 보고 `IGNORED` 처리합니다.
