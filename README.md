...
주문 생성 이후 이어지는 payment · settlement · notification · outbox 흐름을  
명시적 상태 전이, failure branching, recovery path 중심으로 제어하고 추적합니다.

이 프로젝트는 pinned 대표 포트폴리오 레포로,  
"커머스 주문 이후 흐름을 어떻게 설계했고, 어디까지 검증했는가"를  
...
- **무엇을 증명하나:**  
  explicit state transition, failure branching, compensation, admin reprocessing, retry processor, Modulith boundary, Flyway + integration-test 기준선을 함께 보여줍니다.
- **어디를 보면 되나:**  
...
- settlement 실패와 notification 실패를 같은 오류로 뭉개지 않고 서로 다른 보상 / 재처리 경로로 다루는 것
...
- property-gated scheduler 또는 admin-triggered batch endpoint 추가
...
- `AUTO_RETRY`  
  ...
  due 시점이 된 이벤트는 `NotificationRetryProcessor`를 통해 재처리할 수 있습니다.
- `MANUAL_INTERVENTION`  
  운영자 확인이 필요하며 notification event를 `MANUAL_INTERVENTION_REQUIRED`로 남깁니다.
- `IGNORE`  
  현재 범위에서 주문 완료를 막지 않아도 되는 실패로 보고 `IGNORED` 처리합니다.
